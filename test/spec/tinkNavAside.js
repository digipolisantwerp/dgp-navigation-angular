...
describe('navAside', function() {
	beforeEach(module('dgp.dgpApi'));
	beforeEach(module('dgp.navigation'));

	var bodyEl = $('body'), sandboxEl,today,scope,$compile,$templateCache,$location,$window;

	beforeEach(inject(function(_$rootScope_, _$compile_, _$templateCache_,_$location_,_$window_) {
	...
	var templates = {
		'default': {
			scope: {},
			element: '<div data-dgp-nav-aside data-auto-select="false" class="nav-left">'+
				'<aside>'+
		...
		'auto-true': {
			scope: {},
			element: '<div data-dgp-nav-aside data-auto-select="true" class="nav-left">'+
				'<aside>'+
					'<div class="nav-aside-section">'+
				    '<p class="nav-aside-title">Choose a playground</p>'+
				    '<ul>'+
	...
	it('should have no elements selected', function() {
		...
		spyOn($window.dgpApi.util, 'getCurrentURL').and.returnValue('http://localhost:8080/context.html#/forms');
		var elm = compileDirective('default');
			scope.$digest();
			setTimeout(function(){
		...
		it('default should change active', function() {
			spyOn($window.dgpApi.util, 'getCurrentURL').and.returnValue('http://localhost:8080/context.html#/menu1');
			var elm = compileDirective('default');
			scope.$digest();
...
describe('first selected true',function(){
	...
	it('Data accordion enabled open accordion should change active element to first element', function() {
	 		spyOn($window.dgpApi.util, 'getCurrentURL').and.callFake(function(){return 'http://localhost:8080/context.html#/menu2';});
	 		var elm = compileDirective('auto-true');
	 		scope.$apply();
