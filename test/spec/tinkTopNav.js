...
describe('TopNavigation', function() {
  ...
  var $compile, $templateCache, $animate, scope, today, $timeout,$window,dgpApi;

  beforeEach(module('dgp.dgpApi'));
  beforeEach(module('dgp.navigation'));


  beforeEach(inject(function (_$rootScope_, _$compile_, _$templateCache_, _$animate_, _$timeout_,_$window_,_dgpApi_) {
  ...
  var templates = {
    'default': {
      ...
      element: '<nav data-dgp-top-nav class="nav-top">'+
    ...
    'withSidenav': {
      ...
      '<a href="#" title=""><span>My App</span></a>'+
      '</li>'+
      '<li class="nav-top-toggle" data-dgp-sidenav-collapse="sidenav">'+
      '<a href="#" title="Open menu" data-ng-click="sidenav.open = !sidenav.open"><i class="fa fa-bars"><span class="sr-only">Open menu</span></i></a>'+
      '</li>'+
      '<li class="nav-top-app">'+
      '<h1><a href="#">My App</a></h1>'+
      ...
      '</div>'+
      '</nav>'+
      '<aside data-dgp-nav-aside data-toggle-id="sidenav" class="nav-left">'+
  ...
  describe('with sideNavigation', function() {
    ...
    it('when click on toggle expect function to be called',function(){
      var elm = compileDirective('withSidenav');
      var spyToggle = spyOn(dgpApi.sideNavToggle,'toggleById');
      elm.find('li.nav-top-toggle').triggerHandler('click');
      scope.$digest();
